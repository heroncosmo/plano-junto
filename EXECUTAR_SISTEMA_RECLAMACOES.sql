...
-- View para reclamações com dados completos
CREATE OR REPLACE VIEW public.complaints_detailed AS
SELECT 
    c.*,
    g.name as group_name,
    g.services as group_service,
...
GROUP BY c.id, g.name, g.services, u.full_name, a.full_name;
...
BEGIN
    ...
    GROUP BY c.id, g.name, c.problem_type, c.status, c.created_at
...
BEGIN
    ...
    GROUP BY c.id, g.name, p.full_name, c.problem_type, c.status, c.created_at
    ORDER BY c.created_at DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
